...
1. Vérification des conditions de déblocage de nouveaux Tiers/Services/Mails. (Peut être bloquant si une modale de mails est ouverte).
...
*Note : Les unités de temps du modèle sont en secondes (`s`) ou inverse de secondes (`s⁻¹`). Les "trimestres" sont une unité arbitraire de temps long pour les KPI.*
...
Pour chaque projet :
1. **Vérification des requirements** : Pour chaque type dans `serviceTypeRequirements` (et potentiellement `serviceRequirements` stricts), vérifier qu'il y a au moins un service en `running` assigné à ce projet.
...
- Lors d'un crash, le service passe en `crashed`, son yield devient 0 (ce qui met souvent tout le projet à 0 par la règle du `MIN()`).
...
### Migrations Forcées
Les projets ont des versions (v1, v2). À des moments précis (ticks ou events), une migration forcée est déclenchée. Les `serviceTypeRequirements` changent subitement (ex: obligation d'ajouter un `message-queue`). Le projet tombe à 0 K/s tant que le joueur n'achète et n'assigne pas le nouveau service requis.
