...
| `sell`               | *(aucune)*                                 | Instance supprimée, services assignés → `status = stopped` + `hardwareInstanceId = null`, Kouraks += `sellValue` |
...
| `configure(hardwareInstanceId?, projectInstanceId?)` | Au moins un param fourni                                            | Si `hardwareInstanceId` change → `status = deploying`, `remainingDeployTime = deployTime` (timer figé tant que hardware pas `running`) ; sinon status inchangé. Mise à jour de `projectInstanceId` si fourni. |
